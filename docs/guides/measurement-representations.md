...
and verify Core's response metadata on every read, including canonical reads.
They return values already converted by Core; callers must not convert them again.
Missing representation or unit metadata is rejected, even when the response is empty.

## IEC Upgrade

1. Deploy the new Core before starting connectors built with the updated client.
   They require representation and unit metadata on all reads. The Flyway
