...
public class HttpPegelHubClientTest {
    ...
    class MeasurementClientTest {
        ...
        @Test
        void getMeasurementsFailsTheWholeReadWhenAChildFails() throws IOException {
            var pages = new ArrayDeque<>(List.of(
                    measurementListResponse(uuid, true, READ_FROM, 0),
                    measurementListResponse(uuid, false, READ_FROM, 1)));
            when(httpClient.execute(any(), any(HttpClientResponseHandler.class))).thenAnswer(answer -> {
                var request = (org.apache.hc.client5.http.classic.methods.HttpUriRequestBase) answer.getArgument(0);
                var callback = (HttpClientResponseHandler<?>) answer.getArgument(1);
                boolean token = request.getUri().toString().contains("keycloak.local");
                if (!token && pages.isEmpty()) throw new IOException("right child unavailable");
                String body = token ? "{\"access_token\":\"token\",\"expires_in\":300}" : pages.removeFirst();
