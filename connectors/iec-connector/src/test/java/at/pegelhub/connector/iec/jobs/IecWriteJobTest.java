...
class IecWriteJobTest {
    ...
    void shouldSendOnlyWhenLatestMeasurementIsPresent() {
        ...
        PegelHubClient c1 = mock(PegelHubClient.class);
        PegelHubClient c2 = mock(PegelHubClient.class);

        when(reg.coreToProtocolIoas()).thenReturn(Set.of(11, 22, 33));

        when(reg.getCoreToProtocolClient(11)).thenReturn(Optional.of(c1));
        ...
        when(reg.getCoreToProtocolClient(22)).thenReturn(Optional.of(c2));
        ...
        when(reg.getCoreToProtocolClient(33)).thenReturn(Optional.empty());
