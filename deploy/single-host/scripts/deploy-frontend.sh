...
compose() {
  ...
  PEGELHUB_FRONTEND_HOSTNAME="$PEGELHUB_FRONTEND_HOSTNAME" \
  PEGELHUB_API_HOSTNAME="$PEGELHUB_API_HOSTNAME" \
  PEGELHUB_KEYCLOAK_HOSTNAME="$PEGELHUB_KEYCLOAK_HOSTNAME" \
  PEGELHUB_HTTP_BIND="$PEGELHUB_HTTP_BIND" \
  PEGELHUB_HTTPS_BIND="$PEGELHUB_HTTPS_BIND" \
  PEGELHUB_HTTPS_URL_SUFFIX="$PEGELHUB_HTTPS_URL_SUFFIX" \
  PEGELHUB_HTTPS_CONTAINER_PORT="$PEGELHUB_HTTPS_CONTAINER_PORT" \
...
smoke_frontend() {
  ...
  for url in \
    "${FRONTEND_BASE_URL%/}/" \
    "${API_BASE_URL%/}/api/v1/measurements/system-time"; do
    attempt=1
...
PEGELHUB_API_HOSTNAME=$(file_value "$ENV_FILE" PEGELHUB_API_HOSTNAME)
PEGELHUB_KEYCLOAK_HOSTNAME=$(file_value "$ENV_FILE" PEGELHUB_KEYCLOAK_HOSTNAME)
PEGELHUB_HTTP_BIND=$(file_value "$ENV_FILE" PEGELHUB_HTTP_BIND)
[ -n "$PEGELHUB_HTTP_BIND" ] || PEGELHUB_HTTP_BIND=80
PEGELHUB_HTTPS_BIND=$(file_value "$ENV_FILE" PEGELHUB_HTTPS_BIND)
[ -n "$PEGELHUB_HTTPS_BIND" ] || PEGELHUB_HTTPS_BIND=443
PEGELHUB_HTTPS_URL_SUFFIX=$(file_value "$ENV_FILE" PEGELHUB_HTTPS_URL_SUFFIX)
PEGELHUB_HTTPS_CONTAINER_PORT=$(file_value "$ENV_FILE" PEGELHUB_HTTPS_CONTAINER_PORT)
[ -n "$PEGELHUB_HTTPS_CONTAINER_PORT" ] || PEGELHUB_HTTPS_CONTAINER_PORT=443
...
[ -n "$PEGELHUB_API_HOSTNAME" ] || fail "PEGELHUB_API_HOSTNAME is required."
[ -n "$PEGELHUB_KEYCLOAK_HOSTNAME" ] || fail "PEGELHUB_KEYCLOAK_HOSTNAME is required."
FRONTEND_BASE_URL=${FRONTEND_BASE_URL:-https://$PEGELHUB_FRONTEND_HOSTNAME$PEGELHUB_HTTPS_URL_SUFFIX}
API_BASE_URL=${API_BASE_URL:-https://$PEGELHUB_API_HOSTNAME$PEGELHUB_HTTPS_URL_SUFFIX}
