...
21 application presents Core's operator monitoring views and measurement history; it does not own
or persist domain data. [PegelHub Core](../core/README.md) supplies the data and enforces access.
...
The application provides a filterable time-series overview and a detail view with station and
measuring-point metadata, the latest available reading within the preceding 365 days, and bucketed
chart history. Metadata administration is available through the Core API, but is not implemented
in this frontend. The user interface is German; source code and technical documentation are
English.
...
- Docker with Compose v2 for the [Core local stack](../core/README.md#local-docker-stack) and image validation
- `curl` for Core startup and image validation

Chromium is needed only for browser tests, not for `npm start` or the unit suite.
Install the Playwright-managed browser after installing npm dependencies, as shown under
...
**Messreihen** page after sign-in and loads its data through the frontend's `/api/v1` path. A new
Core database has no station metadata or readings, so an empty overview is expected until data
is provisioned. The [Bruno write workflow](../core/docs/api/bruno/README.md#write-workflow) can
...
| `/overview`               | Lists active time series with context, filters, and the latest reading found within 365 days.   |
| `/overview/:timeSeriesId` | Shows one series, metadata, its latest available reading, status, and selectable chart history. |
| `/forbidden`              | Displays the authorization error reached after a Core API `403` response.                       |

Core omits inactive station, measuring-point, or time-series paths from the overview. A direct
detail link still opens an inactive series, marks it as inactive, and retains its history.
When no reading exists within the 365-day lookup window, the UI shows missing data, not a zero value.
Water-level detail charts can optionally show RNW and HSW reference levels when Core supplies
...
The monitoring API requires a `USER` actor with both `metadata:read` and `measurement:read`, or
a `USER` actor with `system:admin`. A connector token is not an operator session. Core remains
responsible for authorization. An API `401` starts a new login while preserving the
current route; an API `403` navigates to `/forbidden`. At token expiry, the Keycloak integration
attempts refresh for active sessions. At that check, 30 minutes without tracked user activity,
or a failed refresh, triggers a new login; this is not a separate 30-minute logout timer.
...
detection, and lazy-loaded feature routes. PrimeNG provides controls, AG Grid Community the
overview grid, and Chart.js the measurement charts.

`MonitoringApiService` requests `/monitoring/time-series` and
`/monitoring/time-series/:id` with `latestWithin=365d`. Core assembles those responses from
metadata and measurements; the browser does not join metadata collections or issue one
latest-measurement request per overview row. Chart history is loaded separately from
`/time-series/:id/measurements/buckets` by `MeasurementApiService`.

```text
src/main.ts                              theme initialization, runtime config, bootstrap
src/app/core/                            config, auth, HTTP/API clients, theme, formatting
src/app/shell/                           navigation shell, account controls, partner marks
...
| `npm run test:browser`   | Run the monitoring route suite in headless Chromium; requires Playwright's browser install.      |
...
| `npm run check`          | Run formatting, both TypeScript checks, and the unit suite; excludes browser tests and coverage. |
| `npm run build`          | Create the production bundle in `dist/pegelhub-frontend-next/browser/`.                          |
...
and Core responses. `npm run smoke:live` makes HTTP requests against a running frontend, Keycloak,
and Core stack; it is not an end-to-end browser login test. The live smoke checks metadata
collections and their relationships, plus raw and bucketed measurements for the first available
series. It skips measurement reads when no series exists and does not currently exercise the
monitoring read-model endpoints.
...
These commands run from `frontend/`. The browser installation is a one-time setup per
Playwright version; CI uses `npx playwright install --with-deps chromium` to install Linux
system dependencies as well. CI also runs `npm run test:coverage` for a diagnostic report.
...
[single-host runbook](../deploy/single-host/README.md#first-installation).
...
- every pull request runs dependency installation, formatting and type checks, unit tests,
  diagnostic coverage, Chromium browser tests, a production build, and container-image validation
...
[single-host runbook](../deploy/single-host/README.md#updates-rollback-and-health) for the
...
Confirm that the token identifies a `USER` actor and has both `metadata:read` and `measurement:read`
on the `pegelhub-core-api` client, or `system:admin`. After changing role assignments, sign out and
back in so the browser receives fresh token claims.
...
- [Operator station metadata note](docs/operator-station-metadata.md): source-data observations for
  station metadata; it is not the runtime API contract or an administration UI guide.
...
- [Single-host deployment](../deploy/single-host/README.md#updates-rollback-and-health):
