...
public record MeasurementWriteTarget(TimeSeries timeSeries, MeasuringPoint measuringPoint, Station station) {
    public MeasurementWriteTarget {
        ...
        if (!timeSeries.measuringPointId().equals(measuringPoint.id())
            ...
            throw new IllegalArgumentException("Write target must contain the TimeSeries' own measuring point and station");
        }
    }
}
